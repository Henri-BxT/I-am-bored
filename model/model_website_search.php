...
function quick_search($db_connect, $search, $asc_desc, $type, $name_grade, $media){	
	...
	$tmp = mysqli_fetch_array($REQ, MYSQLI_NUM);
	$SQL = 'SELECT DISTINCT t1.image, t1.title, t2.grade
		FROM '.$media.'s AS t1
		INNER JOIN listed_'.$media.'s AS t2 ON t1.id_'.$media.' = t2.id_'.$media.'
		INNER JOIN '.$media.'_types AS t3 ON t3.id_'.$media.' = t1.id_'.$media.'
		INNER JOIN types AS t4 ON t3.id_type = t4.id_type
